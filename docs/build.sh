#!/usr/bin/env bash
# Build the consensus Sphinx/Shibuya site: org → rst → html
# Usage (from repo root or this directory):
#   ./docs/build.sh
# Optional: CONSENSUS_DOC_EXPORTER=emacs|pandoc  CONSENSUS_DOC_VENV=path
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

VENV="${CONSENSUS_DOC_VENV:-$ROOT/.venv-docs}"
